...
user_data = load_user_data('/Applications/Files/SEM_7/MAJOR/demo/TESTING/NewIdea/DATA/user_data_discrete.pkl')
...
model1.load_weights('/Applications/Files/SEM_7/MAJOR/fine_tuned_model_discrete_random_overwrite.h5')  # Replace with the correct path

# Freeze layers except the last few
# for layer in model1.layers[:-3]:  # Keep the last 3 layers trainable
#     layer.trainable = False
for layer in model1.layers[:-6]:  # Keep the last 3 layers trainable
    layer.trainable = False
...
model1.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=0.00009), metrics=['accuracy'])
...
model1.save('fine_tuned_model_discrete_random_overwrite.h5')
